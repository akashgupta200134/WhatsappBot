...
const CONFIG = {
  ...
  COL_PHONE    : 'PhoneNumber',
  COL_REMARK   : 'Remark',
  COL_NAME     : 'Name',   // optional column used to personalize the greeting
  ...
  // Randomized gap between contacts instead of a fixed delay — a fixed,
  // identical gap every time is an easy "this is a bot" signal.
  DELAY_BETWEEN_CONTACTS_MIN : 25000,   // 25s
  DELAY_BETWEEN_CONTACTS_MAX : 90000,   // 90s
  DELAY_AFTER_IMAGE_MIN      : 3000,
  DELAY_AFTER_IMAGE_MAX      : 6000,
  DELAY_AFTER_MESSAGE_MIN    : 2000,
  DELAY_AFTER_MESSAGE_MAX    : 5000,
  WHATSAPP_LOAD_TIMEOUT  : 25000,
  WA_READY_TIMEOUT       : 600000,  // 10 minutes — time to call, get OTP, and log in

  // {name} gets replaced per-contact. If a contact has no name, it falls
  // back to plain "Hello 👋" automatically — see Helpers.buildMessage().
  MESSAGE:`Hello👋
...
const Helpers = {
  ...
  // Builds the message for one contact. If a name is available it's
  // inserted into the greeting ("Hello Akash 👋"); otherwise the
  // placeholder is removed and it falls back to plain "Hello 👋".
  buildMessage(name) {
    const cleanName = String(name || '').trim();
    const greetingName = cleanName ? ` ${cleanName}` : '';
    return CONFIG.MESSAGE.replace('{name}', greetingName);
  },
};
